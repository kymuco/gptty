...
- проверка auth-файла через `gptty auth status`
- обновление `auth_data.json` через `gptty auth refresh`
...
Проверить текущий auth-файл без открытия браузера:

```bash
gptty auth status
```

JSON output для scripts:

```bash
gptty auth status --format json
```

Обновить auth-данные через CLI wrapper:
...
Legacy script entrypoints всё ещё доступны из checkout:
...
После успешного захвата рядом появится `auth_data.json`. Подробнее про lifecycle и troubleshooting: [docs/auth.md](docs/auth.md).
...
- ChatGPT web-session auth может истекать; на практике его нужно периодически обновлять.
- Запускайте `gptty auth status`, если запросы начали падать или перед длинной terminal-сессией.
...
- В `auto`-режиме auth refresh отправляет одно probe-сообщение для запуска захвата. По умолчанию это `"Hello"`, но текст можно заменить через `--probe-prompt`.
- В `wait`-режиме auth refresh не отправляет probe автоматически. Войдите или зарегистрируйтесь, затем отправьте любое сообщение вручную в браузере, чтобы запустить захват.
- Новые auth captures пишут канонический `accessToken` и legacy-compatible поле `api_key`.
...
- Отсутствует `auth_data.json`
  Запустите `gptty auth refresh --mode wait`, завершите вход в браузере, затем отправьте любое сообщение в окне чата.
...
- Сначала всё работало, а потом запросы перестали проходить
  Скорее всего истекли cookies сессии или `api_key/accessToken`. Сгенерируйте `auth_data.json` заново через `gptty auth refresh --mode wait`.
